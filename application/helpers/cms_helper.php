...
function get_excerpt($article, $numwords = 45){
	$string = '';
	$url = article_link($article);
	$string .= '<h2 style = "color: rgb(103, 188, 230);>' . anchor($url, e($article->title)) .  '</h2>';
	$string .= '<p class="pubdate">' . e($article->pubdate) . ' by <b>Calvin</b></p><hr>';
	...
	if (count($array)) {
		...
		foreach ($array as $item) {
			...
			if (isset($item['children']) && count($item['children'])) {
				...
				$str .= '<a  class="dropdown-toggle" data-hover="dropdown" data-toggle="dropdown" href="' . site_url(e($item['slug'])) . '">' . e($item['title']);
				$str .= '<b class="caret"></b></a>' . PHP_EOL;
				$str .= get_menu($item['children'], TRUE);
			}
			else {
				$str .= $active ? '<li class="active">' : '<li class="inactive">';
